              ...
              <div key={label} className="flex items-center gap-4 md:gap-6">
                ...
                {href
                  ? <a href={href} target={href.startsWith('http') ? '_blank' : undefined} rel="noreferrer"
                      className="font-body-md text-sm md:text-body-lg text-on-surface hover:text-primary transition-colors break-all">{label}</a>
        ...
        <div className="reveal">
          {status === 'success' ? (
            <div className="flex flex-col items-center justify-center h-full gap-6 text-center py-20">
              ...
              <h3 className="font-headline-md text-2xl text-on-surface">Message Sent!</h3>
              <p className="font-body-lg text-on-surface-variant">
                Shukriya! I'll get back to you within 24 hours.
              </p>
              <button
                onClick={() => setStatus('idle')}
                className="px-8 py-3 border border-primary text-primary font-label-caps text-label-caps hover:bg-primary/10 transition-all"
              >
                SEND ANOTHER
              </button>
            </div>
